...
Mat triangulate::triangulate_points(vector <cv::Point2f> f1, vector <cv::Point2f> f2, float scale, Mat P2, Mat P1) {
    ...
    for (int i = 0; i < f1.size(); i++) {
        float x1 = f1[i].x / scale;
        float y1 = f1[i].y / scale;
        float x2 = f2[i].x / scale;
        float y2 = f2[i].y / scale;

        Mat t1 = (P1.row(2)).mul(x1) - P1.row(0);
        t1.copyTo(J.row(0));
        Mat t2 = (P1.row(2)).mul(y1) - P1.row(1);
        t2.copyTo(J.row(1));
        Mat t3 = (P2.row(2)).mul(x2) - P2.row(0);
        t3.copyTo(J.row(2));
        Mat t4 = (P2.row(2)).mul(y2) - P2.row(1);
        t4.copyTo(J.row(3));
